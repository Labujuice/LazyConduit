...
- [x] **Phase 3: GUI Development**
    ...
    - [x] 模型選擇與參數設定界面
    - [x] GUI 呼叫 CLI 邏輯實作
- [ ] **Phase 4: ROS2 Integration (Conduit Node)**
    - [ ] **基礎架構建立**
        - [ ] 實作 `LazyConduitNode` (基於 `rclpy`)
        - [ ] 訂閱者 1: `/lazy_conduit/text_input` (std_msgs/String) - 支援 Markdown 檔案連結語法
        - [ ] 訂閱者 2: `/lazy_conduit/vision_input` (自定義或實作同步機制接收 Image + String)
        - [ ] 發布者: `/lazy_conduit/output` (std_msgs/String) - LLM 回應結果
    - [ ] **功能實作**
        - [ ] 實作 ROS2 訊息轉 Base64 影像邏輯 (CV Bridge)
        - [ ] 整合既存的 `PromptParser` 與 `FileConverter` 到 ROS2 回呼函數
